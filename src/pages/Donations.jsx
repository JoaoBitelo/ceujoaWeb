...
export default class Donations extends React.Component {
    ...
    componentDidMount = async () => {
        ...
        var res = await this.FetchService.getContent("Doacoes");
        if (res === false) {
    ...
    handleSubmit = async () => {
        this.setState({ loading: true });
        var res = await this.FetchService.changeDonation(this.state.tittle, this.state.text);
        if (res === false) {
            alert("Login ou senha incorretos");
        } else if (res === null) {
            await this.ResponseHandler.nullResponse();
